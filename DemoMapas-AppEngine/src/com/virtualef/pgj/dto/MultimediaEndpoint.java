...
@Api(name = "multimediaendpoint", namespace = @ApiNamespace(ownerDomain = "demomapas.com", ownerName = "demomapas.com", packagePath = "model"))
public class MultimediaEndpoint {
	...
	public CollectionResponse<Multimedia> listMultimedia(
		...
		try {
			...
			Query query = mgr
					.createQuery("select from Multimedia as Multimedia");
	...
	private boolean containsMultimedia(Multimedia multimedia) {
		...
		try {
			Multimedia item = mgr.find(Multimedia.class, multimedia.getId());
			if (item == null) {
				contains = false;
			}
		} finally {
